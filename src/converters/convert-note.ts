...
export class NoteConverter extends ANConverter {
  ...
  parseTokens(): ANFragmentPair[] {
    ...
    while (i < this.note.attributeRun.length) {
      ...
      // Split text because Obsidian has formatting issues with:
      //    1. Formatting across line breaks: "**bold\ntext**" -> incorrect
      //    2. Formatting with leading/trailing spaces: " **bold** " -> incorrect
      //    Examples:
      //    - Input:  "  **bold text**  \n  next line"
      //    - Output: ["**bold text**", "next line"]
  ...
  async format(table = false): Promise<string> {
    ...
    let firstLineSkip =
      !table &&
      this.importer.omitFirstLine &&
      this.note.noteText.includes("\n"); // Changed from contains to includes
    let converted = "";
    ...
    for (let j = 0; j < fragments.length; j++) {
      ...
      attr.atLineStart =
        j == 0 ? true : fragments[j - 1]?.fragment.includes("\n"); // Changed from contains to includes
    ...
    if (this.multiRun != ANMultiRun.None)
      converted += this.formatMultiRun({} as ANAttributeRun);
  ...
  async formatHtmlAttr(attr: ANAttributeRun): Promise<string> {
    ...
    if (attr.superscript == ANBaseline.Super)
      attr.fragment = `<sup>${attr.fragment}</sup>`;
    if (attr.superscript == ANBaseline.Sub)
      attr.fragment = `<sub>${attr.fragment}</sub>`;
    ...
    if (
      attr.highlightColor &&
      Object.prototype.hasOwnProperty.call(attr, "highlightColor")
    ) {
      const highlightStyle = this.convertHighlightColor(attr.highlightColor);
      style += `color:${highlightStyle.color};background-color:${highlightStyle.backgroundColor};`;
    }
  ...
  async formatAttachment(attr: ANAttributeRun): Promise<string> {
    ...
    const attachment = await this.importer.resolveAttachment(
      id,
      attr.attachmentInfo!.typeUti
    );
    let link = attachment
  ...
  convertHighlightColor(type: ANHighlightColor): {
    color: string;
    backgroundColor: string;
  } {
    switch (type) {
      case ANHighlightColor.PURPLE:
        return {
          color: "#FF00FF",
          backgroundColor: "#BA55D333",
        };
      case ANHighlightColor.PINK:
        return {
          color: "#FF4081",
          backgroundColor: "#D5000044",
        };
      case ANHighlightColor.ORANGE:
        return {
          color: "#FBC02D",
          backgroundColor: "#FF6F0022",
        };
      case ANHighlightColor.MINT:
        return {
          color: "#8DE5DB",
          backgroundColor: "#289C8ECC",
        };
      case ANHighlightColor.BLUE:
        return {
          color: "#BBDEFB",
...
function isBlockAttachment(attr: ANAttributeRun) {
  ...
  return !attr.attachmentInfo.typeUti.includes(
...
function attrEquals(a: ANAttributeRun, b: ANAttributeRun): boolean {
  ...
  for (const field of a.$type.fieldsArray) {
    ...
    if (field.name == "highlightColor") {
      if (a[field.name] === b[field.name]) continue;
    }
