import {
  ANAttachment,
  ANConverter,
  type ANMergeableDataObject,
  type ANMergableDataProto,
  type ANTableObject,
} from "../proto";
...
export class ScanConverter extends ANConverter {
  ...
  async format(): Promise<string> {
    ...
    for (const object of this.objects) {
      ...
      let file = await this.importer.resolveAttachment(
        row.Z_PK,
        ANAttachment.Scan
      );
      if (!file)
        file = await this.importer.resolveAttachment(row.ZMEDIA, row.ZTYPEUTI);

      if (file) {
        links.push(`![${file.name}](${file.path})`);
      } else {
